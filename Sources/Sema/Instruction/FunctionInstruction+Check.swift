...
extension FunctionTypeReference.Argument {
    func resolve(context: FunctionParsingContext) -> (arguments: [FunctionType.Argument], resolver: (FunctionParsingContext) -> ()) {
        ...
        case .count(let int):
            ...
            if count < 0 {
                context.typeChecker.diagnostics.append(Diagnostic(token: int.token, message: TypeCheckDiagnosticMessage.negativeArgumentCount(int), severity: .error))
                count = 0
            }
...
extension FunctionTypeReference.ReturnValue {
    func resolve(context: FunctionParsingContext) -> [FunctionType.Argument] {
        ...
        case .count(let node):
            ...
            if count < 0 {
                context.typeChecker.diagnostics.append(Diagnostic(token: node.count.token, message: TypeCheckDiagnosticMessage.negativeArgumentCount(node.count), severity: .error))
                count = 0
            }
